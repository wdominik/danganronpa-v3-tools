...
mod tests {
    ...
    #[test]
    fn legacy_array_geometry_still_parses_positionally() {
        // serde maps a JSON array onto a struct's fields positionally, so an
        // old array form is still accepted (x/y, width/height, left/right/
        // vertical in order). This is a harmless migration convenience — the
        // canonical/documented shape is the named object; the enforced break
        // is the `png` → `image_path` rename (see `legacy_png_field_is_rejected`).
        let json = r#"{
          "schema": "drv3-translate/v1",
          "files": [{
            "format": "font", "cpk": "a", "cpk_path": "b", "spc_member": "c",
        ...
        let set = parse(json).expect("array form parses positionally");
        let FileFormat::Font(fg) = &set.files[0].format else {
            panic!("expected a font group");
        };
        let g = &fg.glyphs[0];
